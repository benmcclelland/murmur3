 ...
 * domain.
 *
 * This is an implementation of MurmurHash3_128 with support for progressive
 * processing based on PMurHash implementation written by Shane Day.
 * 
 * Ported to C from: https://github.com/royaltm/node-murmurhash-native
 */
...
/* I386 or AMD64 */
#if defined(_M_I86) || defined(_M_IX86) || defined(_X86_) || defined(__i386__) \
	|| defined(__i386) || defined(i386) || defined(_M_X64) \
...
FORCE_INLINE void doblock128x64(uint64_t *h1, uint64_t *h2, uint64_t *k1,
	uint64_t *k2)
{
...
/* cnt=bytes to process, h1,h2=hash k1,k2=carry, n=bytes in carry, 
 * ptr/len=payload */
FORCE_INLINE void dobytes128x64(int cnt, uint64_t *h1, uint64_t *h2,
	uint64_t *k1, uint64_t *k2, int *n, const uint8_t *ptr, int *len)
{
	for(;cnt--; (*len)--) {
		switch(*n) {
...
/* Finalize a hash. To match the original Murmur3_128x64 the total_length must
 * be provided */
void PMurHash128_Result(const uint64_t * const ph,
	const uint64_t * const pcarry, const uint32_t total_length,
	uint64_t * const out)
{
	uint64_t h1 = ph[0];
	uint64_t h2 = ph[1];
...
/* Main hashing function. Initialise carry[2] to {0,0} and h[2] to an initial
 * {seed,seed} if wanted. Both ph and pcarry are required arguments. */
void PMurHash128_Process(uint64_t * const ph, uint64_t * const pcarry,
